...
einit() {
	...
	then
		...
		then
			...
			echo -e "\033[33mengagementinit\033[31m failed\033[0m. Do you have\033[31m the latest version\033[0m? Continuing..."
			return 1
		fi
	else
...
optimize_config() {
	if [[ $KEEP_CONF == false ]]
	then
		if [[ -f "$HOME/.tmux.conf" && ! `openssl md5 "$HOME/.tmux.conf" | cut -d " " -f 2` == "1612c020abdd7517c59914ff42f57037" ]]
		then
			date=`date "+%F-%R"`
